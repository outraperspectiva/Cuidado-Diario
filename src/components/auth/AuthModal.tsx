...
import { Mail, Lock, User, Eye, EyeOff, ShieldCheck, CheckCircle2, AlertCircle, X, Shield } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleGoogleLogin = async (userEmail?: string, userName?: string) => {
    try {
      ...
      dispatch(showToast({ message: `Bem-vindo, ${user.name}! Conectado via Google.` }));
      setIsGoogleChooserOpen(false);
      if (onClose) onClose();
    } catch (err: any) {
      dispatch(setError(err.message || 'Falha na autenticação Google'));
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-black/45 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden border border-[#DCE3E8] elevation-3 flex flex-col">
        {/* Brand Banner */}
        <div className="bg-[#103557] text-white p-6 text-center relative">
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="absolute right-4 top-4 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors"
              title="Fechar"
            >
              <X className="w-4 h-4" />
            </button>
          ...
          <div className="mx-auto mb-3 flex justify-center">
            <CuidadoDiarioLogo size={52} showBg />
          </div>
          <h2 className="text-xl font-bold tracking-tight">Cuidado Diário</h2>
          <p className="text-xs text-[#E2F0FD]/80 mt-1 max-w-xs mx-auto">
            Diário de Saúde, Dor e Qualidade de Vida
          </p>

          {/* Mode Switcher */}
          <div className="flex bg-[#001D36] p-1 rounded-full max-w-xs mx-auto mt-5 text-xs font-semibold">
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={() => { setMode('cadastro'); setFieldErrors({}); }}
              className={`flex-1 py-1.5 rounded-full transition-all ${
                mode === 'cadastro'
                  ? 'bg-white text-[#103557] font-bold shadow-xs'
                  : 'text-white/70 hover:text-white'
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {error && (
            <div className="p-3 rounded-xl bg-[#FFDAD6]/40 border border-[#D96B5B]/30 flex items-start gap-2 text-xs text-[#93000A]">
          ...
          <div className="space-y-2">
            <button
              ...
              disabled={isLoading}
              className="w-full h-11 rounded-2xl bg-white hover:bg-[#F6F8FA] border border-[#DCE3E8] hover:border-[#103557] transition-all flex items-center justify-center gap-2.5 px-3 shadow-2xs active:scale-98 cursor-pointer disabled:opacity-50"
            >
              ...
              <span className="text-xs font-bold text-[#101D26]">
            ...
            <div className="relative flex py-1 items-center">
              ...
              <span className="shrink mx-2 text-[10px] font-semibold text-[#73777F] uppercase tracking-wider">
                ou com e-mail
              </span>
              <div className="grow border-t border-[#DCE3E8]"></div>
            </div>
          ...
          <div>
            ...
            {/* Password Criteria Checklist in Cadastro Mode */}
              ...
              <div className="mt-2.5 p-2.5 rounded-xl bg-[#F6F8FA] border border-[#DCE3E8] space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-1 text-[10px] text-[#53606B]">
                  ...
                  <span className={passwordStrength.hasUppercase && passwordStrength.hasLowercase ? 'text-[#003B2E] font-semibold' : ''}>
                    {passwordStrength.hasUppercase && passwordStrength.hasLowercase ? '✓' : '•'} Maiúsculas & minúsculas
                  </span>
                  <span className={passwordStrength.hasNumber ? 'text-[#003B2E] font-semibold' : ''}>
                    {passwordStrength.hasNumber ? '✓' : '•'} Pelo menos um número
                  </span>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full h-11 rounded-full bg-[#103557] hover:bg-[#2B4C6F] text-white text-xs font-bold transition-all shadow-sm active:scale-98 flex items-center justify-center gap-2 disabled:opacity-50 mt-2"
          >
            {isLoading ? (
              ...
              <>
                ...
                <span>{mode === 'login' ? 'Entrar com Segurança' : 'Criar Perfil de Saúde'}</span>
              </>
            )}
          </button>

          {/* Demo Login Button */}
          <div className="pt-2 border-t border-[#DCE3E8] text-center">
            <button
              type="button"
              onClick={handleDemoLogin}
              className="w-full py-2.5 px-3 rounded-full bg-[#E2F0FD] hover:bg-[#B7E7F7]/50 text-[#103557] text-xs font-bold transition-all border border-[#B7E7F7] flex items-center justify-center gap-1.5"
            >
              <CheckCircle2 className="w-3.5 h-3.5 text-[#356572]" />
              <span>Entrar com Perfil Demo (Fábio Fernandez)</span>
            </button>
            <p className="text-[10px] text-[#73777F] mt-2">
              Privacidade médica e isolamento por UID garantidos pela arquitetura Firestore ABAC.
            </p>
          </div>
        </form>
        ...
        {isGoogleChooserOpen && (
          <div className="fixed inset-0 z-60 flex items-center justify-center p-3 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-sm rounded-3xl overflow-hidden border border-[#DCE3E8] elevation-4 flex flex-col p-5 animate-in zoom-in-95 duration-150">
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#53606B]">para autenticar no Cuidado Diário</p>
                </div>
              </div>
